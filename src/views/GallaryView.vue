<template>
  <div class="min-h-screen bg-white px-6 py-12">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
      <div v-for="(item, index) in gallery" :key="index" class="overflow-hidden rounded-lg shadow hover:scale-105 transition">
        <img
          :src="item.url"
          :alt="item.title"
    ...
    <div v-if="showLightbox" class="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
      <button @click="closeLightbox" class="absolute top-4 right-4 text-white text-2xl font-bold">&times;</button>
      <img :src="gallery[currentIndex].url" class="max-h-full max-w-full rounded-lg" />
      <button @click="prev" class="absolute left-4 text-white text-3xl font-bold">&lt;</button>
      <button @click="next" class="absolute right-4 text-white text-3xl font-bold">&gt;</button>
    </div>
  </div>
</template>
